...
/// <summary>
/// Base de testes de integracao do modulo Transparencia sobre SQLite em memoria: abre uma unica
/// conexao por classe de teste (o schema vive enquanto a conexao estiver aberta) e cria contextos
/// com os interceptors de tenant e de auditoria, a imagem de PatrimonioTestBase.
/// </summary>
public abstract class TransparenciaTestBase : IDisposable
{
    ...
    protected TransparenciaTestBase()
    {
        _connection = new SqliteConnection("DataSource=:memory:");
        _connection.Open();
    }

    /// <summary>Cria um <see cref="TransparenciaDbContext"/> no contexto do tenant informado, com auditoria e filtro por tenant.</summary>
    /// <param name="tenantId">Tenant cujo escopo o contexto enxerga.</param>
    /// <returns>Contexto pronto (schema criado).</returns>
    protected TransparenciaDbContext CriarContexto(Guid tenantId)
    {
        ...
        var contexto = new TransparenciaDbContext(options, tenantContext);
