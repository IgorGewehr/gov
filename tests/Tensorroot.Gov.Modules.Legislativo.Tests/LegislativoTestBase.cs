...
/// <summary>
/// Base de testes de integracao do modulo Legislativo sobre SQLite em memoria: abre uma unica
/// conexao por classe de teste (o schema vive enquanto a conexao estiver aberta) e cria contextos
/// com os interceptors de tenant e de auditoria, a imagem de PatrimonioTestBase. Usa o
/// <see cref="LegislativoDbContext"/> gerado.
/// </summary>
public abstract class LegislativoTestBase : IDisposable
{
    /// <summary>Tenant A — usado como tenant principal (Camara) dos cenarios.</summary>
    ...
    protected LegislativoTestBase()
    {
        _connection = new SqliteConnection("DataSource=:memory:");
        _connection.Open();
    }

    /// <summary>Cria um <see cref="LegislativoDbContext"/> no contexto do tenant informado, com auditoria e filtro por tenant.</summary>
    /// <param name="tenantId">Tenant cujo escopo o contexto enxerga.</param>
    /// <returns>Contexto pronto (schema criado).</returns>
    protected LegislativoDbContext CriarContexto(Guid tenantId)
    {
        var tenantContext = new TenantContextFake(tenantId);
        var options = new DbContextOptionsBuilder<LegislativoDbContext>()
        ...
        var contexto = new LegislativoDbContext(options, tenantContext);
