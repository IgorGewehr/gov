...
/// <summary>
/// Módulo Patrimonio: registra o DbContext (provider por configuração), interceptors, repositórios,
/// handlers (MediatR) e validators; expõe os endpoints. Ativável por tenant via descoberta no ApiHost.
/// </summary>
public sealed class PatrimonioModule : IModule
{
    /// <inheritdoc />
    public string Name => "Patrimonio";
    ...
    public void AddModule(IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddDbContext<PatrimonioDbContext>((serviceProvider, options) =>
        {
    ...
    /// <inheritdoc />
    public void MapEndpoints(IEndpointRouteBuilder endpoints) => PatrimonioEndpoints.Map(endpoints);
    ...
    public async Task MigrarBancoAsync(string connectionString, string provider, Guid tenantId, IServiceProvider serviceProvider, CancellationToken cancellationToken)
    {
        ...
        var construtor = new DbContextOptionsBuilder<PatrimonioDbContext>();
        if (ehSqlServer)
        {
        ...
        await using var contexto = new PatrimonioDbContext(construtor.Options, SistemaTenantContext.Instancia);
    ...
    public async Task DrenarOutboxAsync(IServiceProvider serviceProvider, CancellationToken cancellationToken)
    {
        ...
        var contexto = serviceProvider.GetRequiredService<PatrimonioDbContext>();
