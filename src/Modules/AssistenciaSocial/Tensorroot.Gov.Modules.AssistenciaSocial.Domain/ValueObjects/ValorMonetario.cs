...
public sealed class ValorMonetario : ValueObject
{
    ...
    /// <summary>Calcula uma fracao deste valor (ex.: 1/4 ou 1/2 do salario minimo).</summary>
    /// <param name="fator">Fator nao-negativo (ex.: 0.25m para um quarto).</param>
    /// <returns>O valor da fracao.</returns>
    public ValorMonetario Fracao(decimal fator)
    {
        ArgumentOutOfRangeException.ThrowIfNegative(fator);
        return new ValorMonetario(decimal.Round(Valor * fator, 2, MidpointRounding.AwayFromZero));
    }
